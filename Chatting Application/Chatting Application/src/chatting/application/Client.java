...
public class Client implements ActionListener//3.Jframe class in swing that contain methods to make frames
{
     JTextField text;//37. we need to display whatever is in our textbox so it has to get displayed gloably
     static JPanel a1;//40.We have to append our text over this jpanel so it has to global as well
     static Box vertical = Box.createVerticalBox();//43. Will set your messages vertically
     
     static JFrame f = new JFrame();
     
     static DataOutputStream dout;
     Client()//2.constructor will be called as soon as object is made
    {
...
//     JLabel back = new JLabel(i1); //15.Explanation later
     ...
     p1.add(back);//16.add() will add the image behind the panel so use p1.add() which will place it over image
      ...
      ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/2.png"));//23.For profile picture , everything 
        ...
        JLabel name = new JLabel("Bunty");//27.JLabel is used to add name on your frame
...
//        JPanel a1 = new JPanel();//31.Making boundary marking for our text area 
     ...
     f.setLocation(800,50);//6.by default the frane open at origin but this helps to open it at particular location
...
//      JLabel output = new JLabel( out );
    ...
    {
    new Client();//1.annonymous object
     try {
